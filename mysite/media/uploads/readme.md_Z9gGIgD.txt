...
10.12 화살표 키를 누를 때 버튼, 슬라이더, 인풋필드 들이 선택되는 문제
   Unity의 UI 시스템에서는 기본적으로 키보드의 화살표 키를 사용하여 UI 요소들 간의 포커스를 이동할 수 있게 되어 있습니다.
1. Navigation 설정을 None으로 변경 (UI 컴포넌트 수동 설정)
각 UI 버튼, 슬라이더 등의 Selectable 컴포넌트에 있는 Navigation 설정을 수정해 화살표 키에 의한 포커스 이동을 방지할 수 있습니다.

각 버튼 또는 슬라이더에 있는 Button이나 Slider 컴포넌트를 선택합니다.
Inspector에서 Navigation 항목을 찾습니다.
기본값은 "Automatic"으로 되어 있는데, 이를 None으로 변경합니다.
  2. SetNavigationNone 스크립트가 Assets > Editor 폴더에 만들고 
실행 절차:

Unity 에디터 상단 메뉴에서 Tools로 이동합니다.
Set Navigation to None 항목을 클릭합니다.